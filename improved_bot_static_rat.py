...
def calculate_expected_sensing_utility(grid, bot_position, rat_probabilities, alpha):
    # Estimate the expected utility of sensing by predicting the reduction in entropy
    current_entropy = -sum(p * math.log(p + 1e-9) for p in rat_probabilities.values())
    
    # Define a reference position (e.g., the most probable rat position) and calculate distance
    reference_position = max(rat_probabilities, key=rat_probabilities.get)
    distance = abs(bot_position[0] - reference_position[0]) + abs(bot_position[1] - reference_position[1])  # Manhattan distance
    
    # Predict new probabilities based on sensing outcomes (ping received or not)
    predicted_probs_ping = markov_filter_update_for_utility_calc(grid, list(rat_probabilities.keys()), bot_position, alpha, True, rat_probabilities.copy())
    predicted_probs_no_ping = markov_filter_update_for_utility_calc(grid, list(rat_probabilities.keys()), bot_position, alpha, False, rat_probabilities.copy())
    
    # Calculate expected new entropy
    new_entropy_ping = -sum(p * math.log(p + 1e-9) for p in predicted_probs_ping.values())
    new_entropy_no_ping = -sum(p * math.log(p + 1e-9) for p in predicted_probs_no_ping.values())
    probability_of_ping = math.exp(-alpha * distance) if distance > 0 else 1
    expected_entropy = probability_of_ping * new_entropy_ping + (1 - probability_of_ping) * new_entropy_no_ping
    
    return max(0, current_entropy - expected_entropy)

def calculate_expected_moving_utility(bot_position, most_probable_rat_position, distance):
    # Estimate the expected utility of moving based on distance reduction
    new_distance = abs(bot_position[0] - most_probable_rat_position[0]) + abs(bot_position[1] - most_probable_rat_position[1])
    return max(0, distance - new_distance)

# UCB - most probable rat position
    ...
    while state.movements < timeout_limit:
        ...
        if ucb_movement >= ucb_sensing:
            ...
            if moved:
                ...
                # new_distance = abs(bot_position[0] - most_probable_rat_position[0]) + abs(bot_position[1] - most_probable_rat_position[1])
                # reward = max(0, distance - new_distance) # Positive reward if closer
                new_distance = abs(bot_position[0] - rx) + abs(bot_position[1] - ry)
                reward = max(0, distance - new_distance)  # Positive reward if closer
                action_rewards['movement'] += reward
            action_counts['movement'] += 1
        else:
            # Perform sensing
            # print("Performing sensing only, no movement this iteration.")
...
# UCB - probability improvement latest (static) (Static Latest)
# def main_phase_2(grid, initial_bot_position, rat_initial_position, alpha, state):
#     timeout_limit = 9000
#     possible_rat_positions = [(x, y) for x in range(1, GRID_SIZE - 1) for y in range(1, GRID_SIZE - 1) if grid[x][y] == 0]
#     bot_position = initial_bot_position
